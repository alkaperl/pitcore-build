 ...
 * <ul>
 * <li> `browser:uncompressed` - build uncomprssed browser bundle (`pitcore-*.js`)
 * <li> `browser:compressed` - build compressed browser bundle (`pitcore-*.min.js`)
...
function startGulp(name, opts) {
  ...
  var fullname = name ? 'pitcore-' + name : 'pitcore';
  var files = ['lib/**/*.js'];
  var tests = ['test/**/*.js'];
  var alljs = files.concat(tests);

  var buildPath = './node_modules/pitcore-build/';
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = buildBinPath + 'browserify --require ./index.js:' + fullname + ' --external pitcore-lib -o ' + fullname + '.js';
    } else {
      browserifyCommand = buildBinPath + 'browserify --require ./index.js:pitcore-lib -o pitcore-lib.js';
    }
